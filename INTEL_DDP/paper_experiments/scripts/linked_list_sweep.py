#!/usr/bin/env python3
"""
Sweep ll.out over (training_size, test_ptr_offset) and visualize as a heatmap,
in one file.

ll.out usage:  ./ll.out <training size> <test ptr offset>
It prints (stderr) ~10000 latencies on one line (an "Average:" line, if present,
is skipped). ll.c tests whether the DMP chases a genuine scattered linked list
(create_node offsets each node so consecutive nodes are non-adjacent). Training
walks <training size> nodes with the deref on; the target is the final node,
test_ptr_offset + 1 links past the trained chain. A hit means the DMP kept
following next-pointers past the trained window.

NOTE: ll.c has NO base/NULL control -- the alternation and its assert are
commented out, so every trial is an attack trial. There is no miss-baseline
here; read the map on its own terms, not as an attack-vs-base gap.

Running this:
  1. Sweeps (training_size, test_ptr_offset), pooling sub-CLIP latencies across
     RUNS, and writes ./ll_sweep/summary.csv
  ...
  2. Draws one heatmap: rows = test_ptr, cols = training_size, colour = chosen
     metric. mean/median: lower (darker) = faster = DMP reached that node.
     hit_rate: higher = more hits.

Usage:
    ./ll.py                 # sweep (overwrites) then plot mean
    ./ll.py median          # sweep then plot median
    ./ll.py hit_rate        # sweep then plot hit_rate
...
# ------------------------------------------------------------------ EDIT ME
BINARY         = "../linked_list.out"            # may be "../ll.out"
SWEEP_DIR      = "./linked_list_sweep"
TRAINING_SIZES = list(range(0, 16, 2))   # arg 1 -> heatmap x
TEST_PTRS      = list(range(0, 18))       # arg 2 -> heatmap y (scattered chase decays fast; keep small)
...
def visualize(metric):
    ...
    ax.set_ylabel("test_ptr (links past trained chain)")
    ax.set_title(f"ll (linked list) sweep: {metric}  (no base control)")
    fig.tight_layout()

    out = f"ll_{metric}.png"
