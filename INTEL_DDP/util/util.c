...
// source: https://stackoverflow.com/questions/3893937/sorting-an-array-in-c
...
uint64_t c_sleep(uint64_t duration, uint64_t __trash) {
    ...
    for(int i = 1; i < (duration | (__trash & MSB_MASK)); i++) 
...
void set_doitm(uint64_t core_id) {
    char path[100]; 
    sprintf(path, "/dev/cpu/%lu/msr", core_id); 
    int fd = openat(AT_FDCWD, path, O_RDWR); 
    uint64_t val = 1; 
    pwrite64(fd, &val, 8, 6913);
    close(fd);
}
...
uint64_t read_doitm(uint64_t core_id) {
    ...
    uint64_t val;
    int status = pread64(fd, &val, 8, 6913); 
    close(fd);
    return val; 
}
